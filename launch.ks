...
IF SHIP:STATUS = "PRELAUNCH" {
    ...
    SET direction TO incField:TEXT:TONUMBER().
    ...
    WRITEJSON(LEX("apo", targetApo, "inc", direction, "profile", missionProfile, "maxQ", maxQLimit), "1:/ascent_config.json").
    g:HIDE(). g:DISPOSE().
    ...
    IF EXISTS("1:/ascent_config.json") {
        ...
        SET direction TO ascentConfig["inc"].
...
IF SHIP:STATUS = "FLYING" AND SHIP:VERTICALSPEED > 0 {
    LOCK STEERING TO HEADING(direction, 90).
}

FUNCTION GetTotalFuel {
    LOCAL totalCur IS 0.
    // Escáner Agresivo: Suma todo lo que sea PROPELENTE (ignora EC, MP, Oxígeno de cabina si RO)
...
FUNCTION logTelemetry {
    ...
    IF TIME:SECONDS > lastLog + 0.5 {
        ...
        LOCAL data IS LEX(
            ...
            "dist", ROUND(SHIP:GEOPOSITION:DISTANCE / 1000, 1), // Downrange (km)
            "hdist", ROUND(SHIP:GEOPOSITION:DISTANCE, 1), // Distance in meters
...
DECLARE FUNCTION PlanificadorInterplanetario {
    ...
    LOCAL missionType IS TERMINAL:INPUT:GETCHAR():TOUPPER().
...
DECLARE FUNCTION EjecutarTransferenciaInterplanetaria {
    ...
    IF dvAvailable < dvRequired {
        ...
        LOCAL confirm IS TERMINAL:INPUT:GETCHAR():TOUPPER().
        IF confirm <> "S" {
...
UNTIL runmode = 0 {
    ...
    IF NOT stage1Separated AND runmode < 3 {
        ...
        if isSeparated {
            ...
            // 4. ENCENDER MOTOR S1 AL 100%
            HUDTEXT("IGNITING S1 ENGINE (100%)", 5, 2, 30, GREEN, FALSE).
            
            // Refine steering for S2 (lighter, more sensitive)
            SET STEERINGMANAGER:MAXSTOPPINGTIME TO 7. // Smoother, slower turns
            SET STEERINGMANAGER:PITCHPID:KD TO 2.0.   // More damping
            ...
            LOCK THROTTLE TO 1.0. // 100% como se solicitó
            WAIT 1.0.
    ...
    IF runmode = 2 {
        ...
        LOCK STEERING TO HEADING(direction, targetPitch).
...
UNTIL salirMenu {
    ...
    SET menuInput TO "".
    UNTIL menuInput <> "" {
        IF TERMINAL:INPUT:HASCHAR { SET menuInput TO TERMINAL:INPUT:GETCHAR(). }
        logTelemetry(SHIP:STATUS).
        WAIT 0.5.
    }
    ...
    ELSE IF menuInput = "4" {
        ...
        UNTIL subSalir {
            ...
            SET subInput TO TERMINAL:INPUT:GETCHAR():TOUPPER().
